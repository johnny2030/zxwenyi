...
class TQwxmenuController extends AdminbaseController {

	private $wxmenu_tq_model;
	
	function _initialize() {
		parent::_initialize();

		$this->wxmenu_tq_model = D( 'Wxmenu_tq' );
	}
	//微信菜单列表
	function index() {
		$where = array();
		...
		$list = $this->wxmenu_tq_model->where($where)->limit( $page->firstRow, $page->listRows )->order("listorder is null,listorder asc")->select();
	...
	function add() {
		if ( IS_POST ) {
		    ...
		    if (empty($_POST['first'])){
                $first = $this->wxmenu_tq_model->where($where)->max('first');
                if (empty($first)){
                    $_POST['first'] = 1;
                }else{
                    $_POST['first'] = $first+1;
	...
	function edit() {
		if ( IS_POST ) {
			...
			$result = $this->wxmenu_tq_model->where(array('id' => $id))->save($_POST);
			if ($result) {
                //记录日志
			...
			$id = intval( I( 'get.id' ) );
            $wxmenu = $this->wxmenu_tq_model->find($id);
			$this->assign($wxmenu);
			$this->display();
		}
	}
    ...
    function delete() {
        if ( isset( $_POST['ids'] ) ) {//批量逻辑删除
            ...
            $result = $this->wxmenu_tq_model->where($where)->select();
            if(empty($result)){
                $data['del_flg'] = 1;
                $data['update_time'] = date('Y-m-d H:i:s',time());
                if ( $this->wxmenu_tq_model->where( "id in ($ids)" )->save( $data ) !== false ) {
                    //记录日志
            ...
            $data['del_flg'] = 0;
            $data['update_time'] = date('Y-m-d H:i:s',time());
            if ( $this->wxmenu_tq_model->where( "id in ($object)" )->save( $data ) !== false ) {
                //记录日志
            ...
            $object = $_GET['object'];
            $log_id = $_GET['id'];
            if ( $this->wxmenu_tq_model->where( "id in ($object)" )->delete() !== false ) {
                //记录日志
            ...
            if(empty($result)){
                $data['del_flg'] = 1;
                $data['update_time'] = date('Y-m-d H:i:s',time());
                if ( $this->wxmenu_tq_model->where(array('id' => $id))->save($data) !== false ) {
                    //记录日志
    ...
    public function listorders() {
        $status = parent::_listorders( $this->wxmenu_tq_model );
        if ( $status ) {
            $this->success('排序成功');
        } else {
            $this->error('排序失败');
        ...
        if ( isset( $_POST['ids'] )){
            ...
            if ($status == 1){
                ...
                if ($result == 0){
                    ...
                    if ( $this->wxmenu_tq_model->where( "id in ($ids)" )->save( $menu_d ) !== false ) {
    ...
    function submenu() {
        ...
        $where['del_flg'] = array('eq',0);
        $list = $this->wxmenu_tq_model->where($where)->order("listorder asc")->select();
        $this->assign( 'id', $id );
        $this->assign( 'list', $list );
        $this->display();
    }
