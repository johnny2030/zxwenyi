...
class HomebasezZJController extends AppframeController {
	...
	function _initialize() {
		...
		if (session('open_id') == "" && !isset($_GET['code'])){
            $redirect_uri=$today->currentUrl();
            $code_url = $wechat->getAuth20Url($redirect_uri);
            redirect($code_url,0);
		}
		if(session('open_id') == "" && isset($_GET['code'])){
		    $code = $_GET['code'];
		    $open_id = $wechat->getOpenID( $code );
            $wechat->getUserInfor( $open_id );
            $where = array();
            ...
            if (empty($user)){
                ...
                if (!empty($data)){
                    D('Common_user')->where(array('open_id' => $open_id))->save($data);
                }
            }
