...
class DoctorController extends CheckController {
    ...
    public function index() {
        ...
        //总资产
        $doctor = $this->common_user_model->field('assets,proportion')->find($id);
        $data['assets'] = $doctor['assets'];

        //今日收入
        $start_time = date('Y-m-d 00:00:00',time());
        $end_time = date('Y-m-d H:i:s',time());
        $where = array();
        $where['payee'] = array('eq',$id);
        $where['pay_status'] = array('eq','1');
        $where['del_flg'] = array('eq','0');
        $where['time'] = array('between',array($start_time,$end_time));
        $profit = $this->common_order_model->where($where)->sum('money');
        if (empty($profit)){
            $data['profit'] = '0.0';
        }else{
            $data['profit'] = $profit * $doctor['proportion'];
        }

        //最近咨询
        $dp_where = array();
        $dp_where['c.doctor_id'] = array('eq',$id);
        $dp_where['c.status'] = array('eq',0);
        $dp_where['c.del_flg'] = array('eq',0);
        $dp_list = $this->common_dp_model->alias('c')->field('distinct c.patient_id,p.*')->join('__COMMON_USER__ p ON c.patient_id=p.open_id')->where($dp_where)->order('c.create_time desc')->select();
